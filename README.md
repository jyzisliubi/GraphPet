...
- **🌐 多 TTS 引擎** — Edge TTS（在线免费）+ Piper TTS（本地离线，隐私友好）
- **🖥 屏幕感知** — 检测全屏应用（游戏/视频）自动隐藏宠物，退出全屏自动恢复
- **🌍 国际化** — 中英双语切换，设置面板一键切语言
...
- ✅ **主题切换**（v0.3.9）— 暗色/亮色/跟随系统
- ✅ **系统托盘完善**（v0.3.9）— 聊天窗口/走动切换/安静模式/关于
- ✅ **多显示器适配**（v0.3.9）— 宠物位置持久化，跨屏走动

### v0.4.0 — TTS 矩阵 + 屏幕感知 + i18n 🎉

- ✅ **多 TTS 引擎** — Edge TTS（在线免费）+ Piper TTS（本地离线，隐私友好）
- ✅ **屏幕感知** — 检测全屏应用（游戏/视频）自动隐藏宠物，退出全屏自动恢复
- ✅ **国际化** — 中英双语切换，轻量零依赖 i18n 方案
- ✅ **14 项 P1 bug 修复** — 拖拽失效/喂食 toast/STT 泄漏/Cubism 4 表情/TTS 节点泄漏/路径穿越/多显示器截屏/ThreadPool shutdown/future 取消
- ✅ **5 项 P2 体验修复** — 滚动劫持/textarea Escape/重复监听/重复欢迎语/restorePetTop 竞态
- ✅ **5 项 P3 性能优化** — 线程池拆分/设置未保存确认/settingsStore race/graphml 缓存/MemoryGraph RAF
